...
```{r}
printCoefmat(readRDS("summary.RDS"), signif.legend = F)
```
:::
...
On peut aussi s'appuyer sur le test de Diebold Mariano. Développer ? 

Grâce à ces méthodes on peut comparer les estimations dans l'échantillon, ou en temps réel. 

- Dans l'échantillon : utilise toutes les données disponibles pour estimer les valeurs au sein de l'échantillon. Ce sont les fitted values qu'on obtient après une régression linéaire. 

- En temps réel : S'appuie sur les prévisions hors échantillon, qui utilise un sous-ensemble de données pour prévoir des valeurs en dehors de cet échantillon. Pour des estimations en temps réel on estime le modèle sur sur un sous ensemble de données, on prévoit une estimation en dehors du modèle, puis on recommence en ajoutant une nouvelle donnée. On obtient une simulation de prévisions. 
...
Notre idée de voir comment on pourrait 
...
$$
PIB_t = (\beta_{0} + \beta_{1} climat\_fr_t + \beta_{2} \Delta climat\_fr_t) \mathbb 1_{t < t_1} +
\\
(\beta_{0}' + \beta_{1}' climat\_fr_t + \beta_{2}' \Delta climat\_fr_t)\mathbb 1_{t\geq t_1} +
\epsilon_t
$$
...
$$
PIB_t = \beta_{0} + \beta_{1} climat\_fr_t + (\beta_{2} \mathbb 1_{t < t_1}+ \beta_{2}' \mathbb 1_{t\geq t_1})\Delta climat\_fr_t +
\epsilon_t
$$
...
TO DO :fixer breakdates piecereg
...
Dans le cas de notre modèle exemple, Bai et Perron détecte une rupture en 2019. Le modèle suivant est alors estimé :

$$
PIB_t = \beta_{0,t}1_{t< 2019} + \beta_{0,t}1_{t \ge 2019} 
+ \beta_{1,t}1_{t< 2019} climat\_fr_t + \beta_{1,t}1_{t \ge 2019} climat\_fr_t + 
\\
\beta_{2,t}1_{t < 2019} diff\_climat\_fr_t + \beta_{2,t}1_{t \ge 2019} diff\_climat\_fr_t + \epsilon_t 
$$
...
TODO ALAIN : Idée graphique : K_b en fonction de b -> validé

TODO Claire : graph des coefs tvlm in sample
...
To do : graph prévision 2023Q1, modèle estimé jusqu'en 2022Q4
