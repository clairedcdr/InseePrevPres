...
-   parler de la période d'estimation

-   ajouter les coefficients estimés dans la formule de la régression

-   Comment évaluer la qualité des modèles

-   TODO à la fin parler de la suite avec modèle général
...
S'appui lui même sur le test de Chow. Propose un algorithme efficace pour trouver les dates de ruptures (package `strucchange`). 
...
On teste : $$
PIB_t = (\beta_{0} + \beta_{1} climat\_fr_t + \beta_{2} diff\_climat\_fr_t) \mathbb 1_{t < t_1} + 
 (\beta_{0}' + \beta_{1}' climat\_fr_t + \beta_{2}' diff\_climat\_fr_t)\mathbb 1_{t\geq t_1} +
\epsilon_t
$$
...
Dans le cas de notre modèle exemple, Bai et Perron détectent une rupture en 2019. Le modèle suivant est alors estimé :
