...
Etude de 25 modèles de prévision de la production manufacturière (?) estimés entre 1990 Q1 et 2019 Q4. On estime dans l'échantillon et en temps réel les différents modèles précédents et on compare les RMSE des modèles par rapport au modèle linéaire. Puis on fait des moyennes par secteur. On obtient les résultats suivants :  

```{r}
library(DT)
stat_is <- readRDS("graphs_atelier/stats_is.RDS")
stat_is <- round(stat_is, digits = 5)
stat_is
```

```{r}
stat_oos <- readRDS("graphs_atelier/stats_oos.RDS")
stat_oos <- round(stat_oos, digits = 5)
stat_oos
```
...
- Out of sample : les modèles espace_état sont meilleurs que le modèle linéaire en terme de RMSE pour la prévision hors échantillon. Pour ce qui est des autres modèles, plus variable d'un modèle à l'autre.
