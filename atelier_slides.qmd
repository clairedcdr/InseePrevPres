...
-   Grâce à des paramètres ou fonctions (comme `lm_fixed_coeff`), on peut fixer les coefficients de certaines variables. Egalement possible pour les régression par morceau.
...
Nous utilisons une version simplifiée sous la forme :
...
En principe, on peut calculer tous les paramètres par une théorie classique des lois normales multivariées mais le calcul devient vite lourd lorsque $n$ augmente. Utiliser certaines techniques permet d'estimer tous les paramètres de manière dynamique.

. . .

Deux opérations classiques : *filtering* et *smoothing*

-   Filtering : $a_{t+1} = E[\alpha_t|Y_t]$ (proche des estimations en temps-réel)

-   Smoothing : $\hat\alpha_t=E[\alpha_t|y]$ (proche des estimations dans l'échantillon)
